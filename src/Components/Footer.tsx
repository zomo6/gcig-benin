...
import {FaFacebook , FaWhatsapp , FaLinkedin , FaPhone ,  FaInstagram, FaEnvelope , FaTwitter, FaHamburger} from 'react-icons/fa';

function Footer() {

    return (
        <div className="flex flex-col py-5 sticky  sm:px-20 px-5 bg-black text-white">
           <div className="sm:flex items-center justify-between ">
                ...
                <div className="sm:flex items-center sm:space-x-10 space-y-8 ml-6 sm:mt-0 mt-10">
                    <div className="flex items-center sm:mt-7 sm:text-xl text-2xl font-medium space-x-6"> 
                    ...
                    <div className="flex space-x-4 items-center ml-3 sm:ml-0">
                        <FaEnvelope className=" text-2xl font-bold animate-bounce text-[#F39200]"/> 
                        <p className="sm:text-xl text-2xl font-medium ">gcigbenin@gmail.com</p>
                    </div>
                </div>

                <ul className="flex mt-10 sm:mt-0 sm:ml-0 ml-10 space-x-8 text-3xl font-medium  ">
            ...
            <div className="grid grid-cols-3 mt-18 gap-5 my-6 ml-8">
                {
                    footerLink.map((foot)=>(
                        <Link key={foot.id} href={foot.path} className='font-serif font-semibold text-lg text-pretty text-white/70'>
                            {foot.title}
                        </Link>
                    ))
                }
